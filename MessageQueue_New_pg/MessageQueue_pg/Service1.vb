...
Public Class EDGEMessageQueue_PG
    ...
    Protected Shared strLogFilePath As String = AppPath + "\" + "EDGEMessageQueueNEWLog_pg.txt"
        ...
        Try
            ...
            For i As Integer = 1 To nSchemas
                ...
                If Not String.IsNullOrEmpty(strServerName) Then
                    ...
                    End If
                    dbad.SelectCommand.CommandText = "SELECT MESSAGE_ID, DOC_TYPE, ACTION_PROGRAM FROM SYS_MESSAGE_QUEUE WHERE STATUS_FLAG='N' and DOC_TYPE <>'' AND ACTION_PROGRAM <>'' ORDER BY CREATED_DATE"
                    ds.Clear()
                    Try
                        dbad.Fill(ds)
                    Catch ex As Exception
                    ...
                    If (ds.Tables(0).Rows.Count > 0) Then
                        For i1 = 0 To ds.Tables(0).Rows.Count - 1
                            If (ds.Tables(0).Rows(i1)("DOC_TYPE") = "FUNCTION") Then
                                ...
                                End If
                                dbad.UpdateCommand.CommandText = "update SYS_MESSAGE_QUEUE set EXECUTION_START=now() WHERE MESSAGE_ID='" & ds.Tables(0).Rows(i1)("MESSAGE_ID") & "'"
                                Try
    ...
    Public Sub execute_dbfunction(ByVal mid1 As String, ByVal fname As String, ByVal strServerName1 As String, ByVal strUserName1 As String, ByVal strPassWord1 As String)
        ...
        Try
            ...
            If (dbad.UpdateCommand.Connection.State = ConnectionState.Closed) Then
                dbad.UpdateCommand.Connection.Open()
            End If
            dbad.UpdateCommand.CommandText = "update SYS_MESSAGE_QUEUE set STATUS_FLAG='Y', STATUS_MESSAGE='SUCCESS',CHANGED_BY='AUTO', CHANGED_DATE=now(),EXECUTION_END=now() WHERE MESSAGE_ID='" & mid1 & "'"
            dbad.UpdateCommand.ExecuteNonQuery()
        ...
        Catch ex As Exception
            ...
            If (dbad.UpdateCommand.Connection.State = ConnectionState.Closed) Then
                dbad.UpdateCommand.Connection.Open()
            End If
            dbad.UpdateCommand.CommandText = "update SYS_MESSAGE_QUEUE set STATUS_FLAG='R', STATUS_MESSAGE='" & Replace(Mid(ex.Message, 1, 3990), "'", "''") & "',CHANGED_BY='AUTO', CHANGED_DATE=now(),EXECUTION_END=now() WHERE MESSAGE_ID='" & mid1 & "'"
            dbad.UpdateCommand.ExecuteNonQuery()
            dbad.UpdateCommand.Connection.Close()
        End Try
