...
{
    public partial class Edge_EmailService_pg : ServiceBase
    {
        ...
        NpgsqlDataAdapter dbad = new NpgsqlDataAdapter();
        ...
        public void SendBulkEmail(string serName, string uName, string pWord)
        {
            ...
            try
            {
                ...
                dbcon = new NpgsqlConnection(serName);
                da = new NpgsqlDataAdapter(selectQuery, dbcon);
                ...
                if (dsNew1.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dsNew1.Tables[0].Rows.Count; i++)
                    {
                        ...
                        //dbcon = new NpgsqlConnection("Provider=OraOLEDB.Oracle;Data Source=" + serName + ";User Id=" + uName + ";Password=" + pWord + ";");
                        da = new NpgsqlDataAdapter(selectQuery, dbcon);
                        ...
                        if (!string.IsNullOrEmpty(strToAdd))
                        {
                            ...
                            if (chkEmail == true)
                            {
                                strUpdate = "UPDATE SYS_GENERIC_EMAIL  SET SEND_CONFIRMATION='Y',CHANGED_BY='ADMN', CHANGED_DATE=now() WHERE SNO ='" + strSno + "'";
                                LogExceptions(strLogFilePath, null, "Mail has been Send to:" + strSno);
                            }
                            else
                            {
                                strUpdate = "UPDATE SYS_GENERIC_EMAIL SET SEND_CONFIRMATION='R',CHANGED_BY='ADMN', CHANGED_DATE=now() WHERE SNO ='" + strSno + "'";
                            }
                        }
                        else
                        {
        ...
        public bool SendEmailWithExchange(string strTo, string strCc, string strBcc, string strFrom, string strSubject, string strBody, string strAttachmentPath, string sName, string sUser, string sPwd)
        {
            ...
            if (!string.IsNullOrEmpty(strFrom))
            {
                ...
                dbcon = new NpgsqlConnection( sName );
                da = new NpgsqlDataAdapter(selectQuery, dbcon);
                dsUser.Clear();
        ...
        public bool SendEmailWithSMTP(string strTo, string strCc, string strBcc, string strFrom, string strSubject, string strBody, string strAttachmentPath, string sName, string sUser, string sPwd)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strFrom))
                {
                    ...
                    string selectQuery = "SELECT E_MAIL,EMAIL_PASSWORD,FIRST_NAME || LAST_NAME as NAME  FROM SYS_USER_MASTER WHERE upper(E_MAIL)=upper('" + strFrom.Trim() + "')";
                    LogExceptions(strLogFilePath, null, selectQuery);
                    dbcon = new NpgsqlConnection( sName );
                    da = new NpgsqlDataAdapter(selectQuery, dbcon);
                    dsUser.Clear();
